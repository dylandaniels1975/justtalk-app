...
import { PaperPlaneRight, Camera, SignOut, Robot } from '@phosphor-icons/react';
...
export default function ChatPage() {
  ...
  const takePolaroid = async () => {
    ...
    try {
      ...
      alert('Polaroid saved!');
    } catch (err) {
      alert(err.response?.data?.detail || 'Failed to save polaroid');
    }
  };
    ...
    <div className="h-screen bg-[#050505] flex flex-col" data-testid="chat-page">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          {partner?.is_ai && <Robot size={18} className="text-[#FF3B30]" />}
          <div>
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1" data-testid="messages-container">
        {conversation?.shared_interests?.length > 0 && (
        ...
        {messages.map((msg, i) => (
          <motion.div
            ...
            className={`py-1.5 ${isMyMessage(msg) ? 'text-right' : ''}`}
